...
std::string LevenbergMarquardtOptimizationStatus::to_string() const {
  ...
  // clang-format off
    if(iterations == 0 && total_inner_iterations == 0) {
      sst << boost::format("%5s %5s %15s %15s %10s %5s %5s %10s %10s\n") % "iter" % "lm" % "error" % "cost_d" % "lambda" % "ldec" % "dec" % "lm_msec" % "time_msec";
    }

    sst << boost::format("%5d %5d %15g %15g %10g %5c %5c %10g %10g") % iterations % total_inner_iterations % error % cost_change % lambda % check[solve_success] % check[dec]  % (lambda_iteration_time * 1000) % (elapsed_time * 1000);
  // clang-format on
